...
public class DisplayUtil {
    ...
    public static int getUsefulScreenWidth(Activity activity) {
        return getUsefulScreenWidth(activity, NotchUtil.hasNotch(activity));
    }

    public static int getUsefulScreenWidth(View view) {
        return getUsefulScreenWidth(view.getContext(), NotchUtil.hasNotch(view));
    }
    ...
    public static int getUsefulScreenWidth(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isLandscape && DeviceUtil.isEssentialPhone()
        ...
        if (isLandscape) {
            ...
            if (DeviceUtil.isHuawei() && !DisplayUtil.huaweiIsNotchSetToShowInSetting(context)) {
                result -= NotchUtil.getNotchSizeInHuawei(context)[1];
            }
